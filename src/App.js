...
class App extends React.Component {
  constructor() {
    ...
    this.state = {
      memoryCards: cardsData.map((dataObj) => (
        <MemoryCard
          image={dataObj.img}
          id={dataObj.id}
  ...
  handleMatchCards() {
    this.openCards.forEach((card) => {
      ...
      this.setState((prevState) => ({
        foundedCards: [...prevState.foundedCards, card],
      }));
    });
  ...
  render() {
    return (
      <>
        ...
        <div className="card-container">{this.state.memoryCards}</div>
        <button onClick={() => this.restart()} className="replay-btn">
